...
int TextHelper::writeWesternPlanets()
{
  ...
  if ( wstyle & MAIN_CHILD_SHOW_HOUSEPOS ) { opt_housepos = true; numcols++; }
  if ( wstyle & MAIN_CHILD_SHOW_DECLINATION ) { opt_declination = true; numcols++; }
...
int TextHelper::writeKp( const int &dasaindex )
{
  ...
  table.col_alignment[1] = Align::Right;
  table.col_alignment[2] = Align::Center;
  table.col_alignment[3] = Align::Center;
  table.col_alignment[4] = Align::Center;
  ...
  for( unsigned int i1 = 0; i1 < obs.size(); i1++ )
  {
    ...
    horoscope->getKPLords( p, &lord, &sublord );
    if ( expert->isRasiDasaExpert() )
    {
      table.setEntry( 4, line, writer->getSignName( lord, TLARGE ) );
      table.setEntry( 5, line, writer->getSignName( sublord, TLARGE ) );
    }
    else
    {
      table.setEntry( 4, line, writer->getObjectName( lord, TLARGE, true ) );
      table.setEntry( 5, line, writer->getObjectName( sublord, TLARGE, true ) );
    }
    line++;
  }
  for( p = HOUSE1; p <= HOUSE12; p++ )
  {
    ...
    horoscope->getHouseKPLords( p, &lord, &sublord );
    if ( expert->isRasiDasaExpert() )
    {
      table.setEntry( 4, line, writer->getSignName( lord, TLARGE ) );
      table.setEntry( 5, line, writer->getSignName( sublord, TLARGE ) );
