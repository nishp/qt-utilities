...
#ifdef __GNUG__
  #pragma implementation "TreeWidget.h"
#endif
...
WX_DEFINE_EXPORTED_ARRAY(MyGenericTreeItem *, MyArrayGenericTreeItems);
...
class MyTreeRenameTimer: public wxTimer
{
public:
    ...
    MyTreeRenameTimer( MyGenericTreeCtrl *owner );

    virtual void Notify();

private:
    MyGenericTreeCtrl *m_owner;
};

// control used for in-place edit
class MyTreeTextCtrl: public wxTextCtrl
{
public:
    MyTreeTextCtrl(MyGenericTreeCtrl *owner, MyGenericTreeItem *item);
...
private:
    MyGenericTreeCtrl  *m_owner;
    MyGenericTreeItem  *m_itemEdited;
...
// timer used to clear MyGenericTreeCtrl::m_findPrefix if no key was pressed
...
public:
    ...
    wxTreeFindTimer( MyGenericTreeCtrl *owner ) { m_owner = owner; }
...
public:
    // ctors & dtor
    MyGenericTreeItem() { m_data = NULL; }
    MyGenericTreeItem( MyGenericTreeItem *parent,
                       const wxString& text,
                       int image,
                       int selImage,
                       MyTreeItemData *data );
    ...
    // trivial accessors
    MyArrayGenericTreeItems& GetChildren() { return m_children; }

    const wxString& GetText() const { return m_text; }
    int GetImage(wxTreeItemIcon which = wxTreeItemIcon_Normal) const
        { return m_images[which]; }
    MyTreeItemData *GetData() const { return m_data; }

    // returns the current image for the item (depending on its
    // selected/expanded/whatever state)
    int GetCurrentImage() const;

	void SetHidden( const bool h ) { hidden = h; }
	bool IsHidden() { return hidden; }
    void SetText( const wxString &text );
    void SetImage(int image, wxTreeItemIcon which) { m_images[which] = image; }
    void SetData(MyTreeItemData *data) { m_data = data; }
    ...
    MyGenericTreeItem *GetParent() const { return m_parent; }

    // operations
        // deletes all children notifying the treectrl about it if !NULL
        // pointer given
    void DeleteChildren(MyGenericTreeCtrl *tree = NULL);

    // get count of all children (and grand children if 'recursively')
    size_t GetChildrenCount(bool recursively = TRUE) const;

    void Insert(MyGenericTreeItem *child, size_t index)
    { m_children.Insert(child, index); }

    void GetSize( int &x, int &y, const MyGenericTreeCtrl* );
    ...
    MyGenericTreeItem *HitTest( const wxPoint& point,
                                const MyGenericTreeCtrl *,
                                int &flags,
                                int level );
...
private:
    ...
    MyTreeItemData     *m_data;         // user-provided data
...
static bool IsDescendantOf(MyGenericTreeItem *parent, MyGenericTreeItem *item)
{
    while ( item )
    {
        if ( item == parent )
...
// -----------------------------------------------------------------------------
// MyTreeRenameTimer (internal)
// -----------------------------------------------------------------------------

MyTreeRenameTimer::MyTreeRenameTimer( MyGenericTreeCtrl *owner )
{
    m_owner = owner;
}

void MyTreeRenameTimer::Notify()
{
    m_owner->OnRenameTimer();
}

//-----------------------------------------------------------------------------
// MyTreeTextCtrl (internal)
//-----------------------------------------------------------------------------

BEGIN_EVENT_TABLE(MyTreeTextCtrl,wxTextCtrl)
    EVT_CHAR           (MyTreeTextCtrl::OnChar)
    EVT_KEY_UP         (MyTreeTextCtrl::OnKeyUp)
    EVT_KILL_FOCUS     (MyTreeTextCtrl::OnKillFocus)
END_EVENT_TABLE()

MyTreeTextCtrl::MyTreeTextCtrl(MyGenericTreeCtrl *owner,
                               MyGenericTreeItem *item)
...
bool MyTreeTextCtrl::AcceptChanges()
{
...
void MyTreeTextCtrl::OnChar( wxKeyEvent &event )
{
...
void MyTreeTextCtrl::OnKeyUp( wxKeyEvent &event )
{
    if ( !m_finished )
    {
...
void MyTreeTextCtrl::OnKillFocus( wxFocusEvent &event )
{
    if ( !m_finished )
    {
...
MyGenericTreeItem::~MyGenericTreeItem()
{
    delete m_data;
...
void MyGenericTreeItem::DeleteChildren(MyGenericTreeCtrl *tree)
{
    size_t count = m_children.Count();
    for ( size_t n = 0; n < count; n++ )
    {
        MyGenericTreeItem *child = m_children[n];
        if (tree)
...
void MyGenericTreeItem::SetText( const wxString &text )
{
    m_text = text;
}

size_t MyGenericTreeItem::GetChildrenCount(bool recursively) const
{
    size_t count = m_children.Count();
    if ( !recursively )
        return count;
...
void MyGenericTreeItem::GetSize( int &x, int &y,
                                 const MyGenericTreeCtrl *theButton )
{
...
MyGenericTreeItem *MyGenericTreeItem::HitTest(const wxPoint& point,
                                              const MyGenericTreeCtrl *theCtrl,
                                              int &flags,
                                              int level)
{
    ...
    if ( !(level == 0 && theCtrl->HasFlag(wxTR_HIDE_ROOT)) )
    {
        ...
        if (m_isCollapsed) return (MyGenericTreeItem*) NULL;
    }
    ...
    for ( size_t n = 0; n < count; n++ )
    {
        MyGenericTreeItem *res = m_children[n]->HitTest( point,
                                                         theCtrl,
                                                         flags,
                                                         level + 1 );
        if ( res != NULL )
    ...
    return (MyGenericTreeItem*) NULL;
}

int MyGenericTreeItem::GetCurrentImage() const
{
    int image = NO_IMAGE;
    if ( IsExpanded() )
    {
...
// -----------------------------------------------------------------------------
// MyGenericTreeCtrl implementation
// -----------------------------------------------------------------------------

IMPLEMENT_DYNAMIC_CLASS(MyGenericTreeCtrl, wxScrolledWindow)

BEGIN_EVENT_TABLE(MyGenericTreeCtrl,wxScrolledWindow)
    EVT_PAINT          (MyGenericTreeCtrl::OnPaint)
    EVT_MOUSE_EVENTS   (MyGenericTreeCtrl::OnMouse)
    EVT_CHAR           (MyGenericTreeCtrl::OnChar)
    EVT_SET_FOCUS      (MyGenericTreeCtrl::OnSetFocus)
    EVT_KILL_FOCUS     (MyGenericTreeCtrl::OnKillFocus)
    EVT_IDLE           (MyGenericTreeCtrl::OnIdle)
END_EVENT_TABLE()

IMPLEMENT_DYNAMIC_CLASS(TreeWidget, MyGenericTreeCtrl)

TreeWidget::TreeWidget(wxWindow *parent, wxWindowID id,
               const wxPoint& pos,
               const wxSize& size,
               long style,
               const wxValidator &validator,
               const wxString& name )
    : MyGenericTreeCtrl(parent, id, pos, size, style, validator, name)
    {
    }
// -----------------------------------------------------------------------------
...
void MyGenericTreeCtrl::Init()
{
    m_current = m_key_current = m_anchor = (MyGenericTreeItem *) NULL;
    m_hasFocus = FALSE;
    m_dirty = FALSE;
    ...
    m_dragCount = 0;
    m_isDragging = FALSE;
    m_dropTarget = m_oldSelection = (MyGenericTreeItem *)NULL;
    m_textCtrl = NULL;
...
bool MyGenericTreeCtrl::Create(wxWindow *parent,
                               wxWindowID id,
                               const wxPoint& pos,
                               const wxSize& size,
                               long style,
...
MyGenericTreeCtrl::~MyGenericTreeCtrl()
{
...
size_t MyGenericTreeCtrl::GetCount() const
{
    return m_anchor == NULL ? 0u : m_anchor->GetChildrenCount();
}

void MyGenericTreeCtrl::SetIndent(unsigned int indent)
{
    m_indent = (unsigned short) indent;
    m_dirty = TRUE;
}

void MyGenericTreeCtrl::SetSpacing(unsigned int spacing)
{
    m_spacing = (unsigned short) spacing;
    m_dirty = TRUE;
}

size_t MyGenericTreeCtrl::GetChildrenCount(const wxTreeItemId& item, bool recursively)
{
    wxCHECK_MSG( item.IsOk(), 0u, wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->GetChildrenCount(recursively);
}

void MyGenericTreeCtrl::SetWindowStyle(const long styles)
{
...
wxString MyGenericTreeCtrl::GetItemText(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxT(""), wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->GetText();
}

int MyGenericTreeCtrl::GetItemImage(const wxTreeItemId& item,
                             wxTreeItemIcon which) const
{
    wxCHECK_MSG( item.IsOk(), -1, wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->GetImage(which);
}

MyTreeItemData *MyGenericTreeCtrl::GetItemData(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), NULL, wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->GetData();
}

wxColour MyGenericTreeCtrl::GetItemTextColour(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxNullColour, wxT("invalid tree item") );

    MyGenericTreeItem *pItem = (MyGenericTreeItem*) item.m_pItem;
    return pItem->Attr().GetTextColour();
}

wxColour MyGenericTreeCtrl::GetItemBackgroundColour(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxNullColour, wxT("invalid tree item") );
...
wxFont MyGenericTreeCtrl::GetItemFont(const wxTreeItemId& item) const
{
...
void MyGenericTreeCtrl::SetItemText(const wxTreeItemId& item, const wxString& text)
{
...
void MyGenericTreeCtrl::SetItemImage(const wxTreeItemId& item,
                              int image,
...
void MyGenericTreeCtrl::SetItemData(const wxTreeItemId& item, MyTreeItemData *data)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );

    ((MyGenericTreeItem*) item.m_pItem)->SetData(data);
}

void MyGenericTreeCtrl::SetItemHasChildren(const wxTreeItemId& item, bool has)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void MyGenericTreeCtrl::SetItemHidden(const wxTreeItemId& item, bool hidden)
{
	MyGenericTreeItem *pItem = (MyGenericTreeItem*) item.m_pItem;
	pItem->SetHidden( hidden );
}

void MyGenericTreeCtrl::SetItemBold(const wxTreeItemId& item, bool bold)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void MyGenericTreeCtrl::SetItemTextColour(const wxTreeItemId& item,
                                   const wxColour& col)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void MyGenericTreeCtrl::SetItemBackgroundColour(const wxTreeItemId& item,
                                         const wxColour& col)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void MyGenericTreeCtrl::SetItemFont(const wxTreeItemId& item, const wxFont& font)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
bool MyGenericTreeCtrl::SetFont( const wxFont &font )
{
...
bool MyGenericTreeCtrl::IsVisible(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), FALSE, wxT("invalid tree item") );

    // An item is only visible if it's not a descendant of a collapsed item
    MyGenericTreeItem *pItem = (MyGenericTreeItem*) item.m_pItem;
    MyGenericTreeItem* parent = pItem->GetParent();
    while (parent)
    {
...
bool MyGenericTreeCtrl::ItemHasChildren(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), FALSE, wxT("invalid tree item") );
    ...
    return ((MyGenericTreeItem*) item.m_pItem)->HasPlus();
}

bool MyGenericTreeCtrl::IsExpanded(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), FALSE, wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->IsExpanded();
}

bool MyGenericTreeCtrl::IsSelected(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), FALSE, wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->IsSelected();
}

bool MyGenericTreeCtrl::IsBold(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), FALSE, wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->IsBold();
}
...
wxTreeItemId MyGenericTreeCtrl::GetItemParent(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );

    return ((MyGenericTreeItem*) item.m_pItem)->GetParent();
}

wxTreeItemId MyGenericTreeCtrl::GetFirstChild(const wxTreeItemId& item, long& cookie) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId MyGenericTreeCtrl::GetNextChild(const wxTreeItemId& item, long& cookie) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );

    MyArrayGenericTreeItems& children = ((MyGenericTreeItem*) item.m_pItem)->GetChildren();
...
wxTreeItemId MyGenericTreeCtrl::GetLastChild(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId MyGenericTreeCtrl::GetNextSibling(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );

    MyGenericTreeItem *i = (MyGenericTreeItem*) item.m_pItem;
    MyGenericTreeItem *parent = i->GetParent();
    if ( parent == NULL )
    {
    ...
    MyArrayGenericTreeItems& siblings = parent->GetChildren();
...
wxTreeItemId MyGenericTreeCtrl::GetPrevSibling(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId MyGenericTreeCtrl::GetNext(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
    ...
    MyArrayGenericTreeItems& children = i->GetChildren();
...
wxTreeItemId MyGenericTreeCtrl::GetFirstVisibleItem() const
{
...
wxTreeItemId MyGenericTreeCtrl::GetNextVisible(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId MyGenericTreeCtrl::GetPrevVisible(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
void MyGenericTreeCtrl::ResetTextControl()
{
  m_textCtrl = NULL;
}

// find the first item starting with the given prefix after the given item
wxTreeItemId MyGenericTreeCtrl::FindItem(const wxTreeItemId& idParent,
...
wxTreeItemId MyGenericTreeCtrl::DoInsertItem(const wxTreeItemId& parentId,
                                      size_t previous,
                                      const wxString& text,
                                      int image, int selImage,
                                      MyTreeItemData *data)
{
    MyGenericTreeItem *parent = (MyGenericTreeItem*) parentId.m_pItem;
    if ( !parent )
    {
    ...
    MyGenericTreeItem *item =
        new MyGenericTreeItem( parent, text, image, selImage, data );

    if ( data != NULL )
    {
        //data->m_pItem = (long) item;
        data->m_pItem = (void*) item;
    }
...
wxTreeItemId MyGenericTreeCtrl::AddRoot(const wxString& text,
                                 int image, int selImage,
                                 MyTreeItemData *data)
{
    ...
    m_anchor = new MyGenericTreeItem((MyGenericTreeItem *)NULL, text,
                                   image, selImage, data);
    if ( data != NULL )
    {
        //data->m_pItem = (long) m_anchor;
        data->m_pItem = (void*) m_anchor;
    }
...
wxTreeItemId MyGenericTreeCtrl::PrependItem(const wxTreeItemId& parent,
                                     const wxString& text,
                                     int image, int selImage,
                                     MyTreeItemData *data)
{
...
wxTreeItemId MyGenericTreeCtrl::InsertItem(const wxTreeItemId& parentId,
                                    ...
                                    MyTreeItemData *data)
{
    ...
    if (idPrevious.IsOk())
    {
        index = parent->GetChildren().Index((MyGenericTreeItem*) idPrevious.m_pItem);
        wxASSERT_MSG( index != wxNOT_FOUND,
                      wxT("previous item in MyGenericTreeCtrl::InsertItem() is not a sibling") );
    }
...
wxTreeItemId MyGenericTreeCtrl::AppendItem(const wxTreeItemId& parentId,
                                    const wxString& text,
                                    int image, int selImage,
                                    MyTreeItemData *data)
{
...
void MyGenericTreeCtrl::SendDeleteEvent(MyGenericTreeItem *item)
{
    MyTreeEvent event( wxEVT_COMMAND_TREE_DELETE_ITEM, GetId() );
    event.m_item = (void*) item;
    event.SetEventObject( this );
    ProcessEvent( event );
}

void MyGenericTreeCtrl::DeleteChildren(const wxTreeItemId& itemId)
{
    m_dirty = TRUE;     // do this first so stuff below doesn't cause flicker

    MyGenericTreeItem *item = (MyGenericTreeItem*) itemId.m_pItem;
    item->DeleteChildren(this);
}

void MyGenericTreeCtrl::Delete(const wxTreeItemId& itemId)
{
    m_dirty = TRUE;     // do this first so stuff below doesn't cause flicker

    MyGenericTreeItem *item = (MyGenericTreeItem*) itemId.m_pItem;

    MyGenericTreeItem *parent = item->GetParent();
...
void MyGenericTreeCtrl::DeleteAllItems()
{
    if ( m_anchor )
    {
        Delete(m_anchor);
...
void MyGenericTreeCtrl::Expand(const wxTreeItemId& itemId)
{
    MyGenericTreeItem *item = (MyGenericTreeItem*) itemId.m_pItem;

    wxCHECK_RET( item, _T("invalid item in MyGenericTreeCtrl::Expand") );
    ...
    MyTreeEvent event( wxEVT_COMMAND_TREE_ITEM_EXPANDING, GetId() );
    event.m_item = (void*) item;
    event.SetEventObject( this );
...
void MyGenericTreeCtrl::ExpandAll(const wxTreeItemId& item)
{
...
void MyGenericTreeCtrl::Collapse(const wxTreeItemId& itemId)
{
    wxCHECK_RET( !HasFlag(wxTR_HIDE_ROOT) || itemId != GetRootItem(),
    ...
    MyTreeEvent event( wxEVT_COMMAND_TREE_ITEM_COLLAPSING, GetId() );
    event.m_item = (void*) item;
    event.SetEventObject( this );
    if ( ProcessEvent( event ) && !event.IsAllowed() )
    {
    ...
    MyArrayGenericTreeItems& children = item->GetChildren();
...
void MyGenericTreeCtrl::CollapseAndReset(const wxTreeItemId& item)
{
    Collapse(item);
    DeleteChildren(item);
}

void MyGenericTreeCtrl::Toggle(const wxTreeItemId& itemId)
{
    MyGenericTreeItem *item = (MyGenericTreeItem*) itemId.m_pItem;
...
void MyGenericTreeCtrl::Unselect()
{
    if (m_current)
    {
...
void MyGenericTreeCtrl::UnselectAllChildren(MyGenericTreeItem *item)
{
    if (item->IsSelected())
    {
...
void MyGenericTreeCtrl::UnselectAll()
{
    ...
    if ( rootItem )
    {
        UnselectAllChildren((MyGenericTreeItem*) rootItem.m_pItem);
    }
}
...
bool MyGenericTreeCtrl::TagNextChildren(MyGenericTreeItem *crt_item, MyGenericTreeItem *last_item, bool select)
{
    MyGenericTreeItem *parent = crt_item->GetParent();

    if (parent == NULL) // This is root item
        return TagAllChildrenUntilLast(crt_item, last_item, select);

    MyArrayGenericTreeItems& children = parent->GetChildren();
...
bool MyGenericTreeCtrl::TagAllChildrenUntilLast(MyGenericTreeItem *crt_item, MyGenericTreeItem *last_item, bool select)
{
    ...
    {
        MyArrayGenericTreeItems& children = crt_item->GetChildren();
        size_t count = children.Count();
        for ( size_t n = 0; n < count; ++n )
        {
...
void MyGenericTreeCtrl::SelectItemRange(MyGenericTreeItem *item1, MyGenericTreeItem *item2)
{
    // item2 is not necessary after item1
    MyGenericTreeItem *first=NULL, *last=NULL;
...
void MyGenericTreeCtrl::SelectItem(const wxTreeItemId& itemId,
                                   bool unselect_others,
                                   bool extended_select)
{
    ...
    MyTreeEvent event( wxEVT_COMMAND_TREE_SEL_CHANGING, GetId() );
    event.m_item = (void*) item;
    event.m_itemOld = (void*) m_current;
    event.SetEventObject( this );
    ...
    if (extended_select)
    {
        if ( !m_current )
        {
            m_current = m_key_current = (MyGenericTreeItem*) GetRootItem().m_pItem;
        }
...
void MyGenericTreeCtrl::FillArray(MyGenericTreeItem *item,
...
size_t MyGenericTreeCtrl::GetSelections(wxArrayTreeItemIds &array) const
{
    array.Empty();
    wxTreeItemId idRoot = GetRootItem();
    if ( idRoot.IsOk() )
    {
        FillArray((MyGenericTreeItem*) idRoot.m_pItem, array);
    }
...
void MyGenericTreeCtrl::EnsureVisible(const wxTreeItemId& item)
{
    if (!item.IsOk()) return;

    MyGenericTreeItem *gitem = (MyGenericTreeItem*) item.m_pItem;

    // first expand all parent branches
    MyGenericTreeItem *parent = gitem->GetParent();
...
void MyGenericTreeCtrl::ScrollTo(const wxTreeItemId &item)
{
    if (!item.IsOk()) return;
...
static MyGenericTreeCtrl *s_treeBeingSorted = NULL;

static int LINKAGEMODE tree_ctrl_compare_func(MyGenericTreeItem **item1,
                                  MyGenericTreeItem **item2)
{
    wxCHECK_MSG( s_treeBeingSorted, 0, wxT("bug in MyGenericTreeCtrl::SortChildren()") );

    return s_treeBeingSorted->OnCompareItems(*item1, *item2);
}

int MyGenericTreeCtrl::OnCompareItems(const wxTreeItemId& item1,
                               const wxTreeItemId& item2)
{
    return wxStrcmp(GetItemText(item1), GetItemText(item2));
}

void MyGenericTreeCtrl::SortChildren(const wxTreeItemId& itemId)
{
    wxCHECK_RET( itemId.IsOk(), wxT("invalid tree item") );
    ...
    wxCHECK_RET( !s_treeBeingSorted,
                 wxT("MyGenericTreeCtrl::SortChildren is not reentrant") );
...
wxImageList *MyGenericTreeCtrl::GetImageList() const
{
    return m_imageListNormal;
}

wxImageList *MyGenericTreeCtrl::GetButtonsImageList() const
{
    return m_imageListButtons;
}

wxImageList *MyGenericTreeCtrl::GetStateImageList() const
{
    return m_imageListState;
}

void MyGenericTreeCtrl::CalculateLineHeight()
{
    wxClientDC dc(this);
    ...
    if ( m_imageListNormal )
    {
        ...
        // May be toggle off. Then MyGenericTreeCtrl will spread when
...
void MyGenericTreeCtrl::SetImageList(wxImageList *imageList)
{
...
void MyGenericTreeCtrl::SetStateImageList(wxImageList *imageList)
{
...
void MyGenericTreeCtrl::SetButtonsImageList(wxImageList *imageList)
{
...
void MyGenericTreeCtrl::AssignImageList(wxImageList *imageList)
{
    SetImageList(imageList);
    m_ownsImageListNormal = TRUE;
}

void MyGenericTreeCtrl::AssignStateImageList(wxImageList *imageList)
{
    SetStateImageList(imageList);
    m_ownsImageListState = TRUE;
}

void MyGenericTreeCtrl::AssignButtonsImageList(wxImageList *imageList)
{
...
void MyGenericTreeCtrl::AdjustMyScrollbars()
{
    if (m_anchor)
    {
        int x = 0, y = 0;
...
int MyGenericTreeCtrl::GetLineHeight(MyGenericTreeItem *item) const
{
...
void MyGenericTreeCtrl::PaintLevel( MyGenericTreeItem *item, wxDC &dc, int level, int &y )
{
    if ( item->IsHidden() ) return;
...
void MyGenericTreeCtrl::DrawDropEffect(MyGenericTreeItem *item)
{
    if ( item )
    {
        if ( item->HasPlus() )
...
void MyGenericTreeCtrl::DrawBorder(const wxTreeItemId &item)
{
    wxCHECK_RET( item.IsOk(), _T("invalid item in MyGenericTreeCtrl::DrawLine") );
...
void MyGenericTreeCtrl::DrawLine(const wxTreeItemId &item, bool below)
{
    wxCHECK_RET( item.IsOk(), _T("invalid item in MyGenericTreeCtrl::DrawLine") );
...
void MyGenericTreeCtrl::OnPaint( wxPaintEvent &WXUNUSED(event) )
{
    wxPaintDC dc(this);
    PrepareDC( dc );
...
void MyGenericTreeCtrl::OnSetFocus( wxFocusEvent &event )
{
    m_hasFocus = TRUE;
...
void MyGenericTreeCtrl::OnKillFocus( wxFocusEvent &event )
{
    m_hasFocus = FALSE;
...
void MyGenericTreeCtrl::OnChar( wxKeyEvent &event )
{
    MyTreeEvent te( wxEVT_COMMAND_TREE_KEY_DOWN, GetId() );
    te.m_evtKey = event;
    ...
    switch ( keyCode )
    {
        ...
        case WXK_RETURN:
            ...
            {
                MyTreeEvent event( wxEVT_COMMAND_TREE_ITEM_ACTIVATED, GetId() );
                event.m_item = (void*) m_current;
                event.SetEventObject( this );
                GetEventHandler()->ProcessEvent( event );
            }
                ...
                if (!prev)
                {
                    ...
                    if (prev)
                    {
                        ...
                        if (current == GetFirstChild( prev, cookie ))
                        {
                            ...
                            m_key_current= (MyGenericTreeItem*) prev.m_pItem;
                            break;
                        }
                    }
                }
        ...
        case WXK_DOWN:
            {
                if (IsExpanded(m_key_current) && HasChildren(m_key_current))
                {
                    ...
                    m_key_current=(MyGenericTreeItem*) child.m_pItem;
                }
                else
                {
                    ...
                    if (next)
                    {
                        ...
                        m_key_current=(MyGenericTreeItem*) next.m_pItem;
                    }
                }
            }
            break;
...
wxTreeItemId MyGenericTreeCtrl::HitTest(const wxPoint& point, int& flags)
{
    ...
    MyGenericTreeItem *hit =  m_anchor->HitTest(CalcUnscrolledPosition(point),
   //    MyGenericTreeItem *hit =  m_anchor->HitTest(point,
                                                this, flags, 0);
    if (hit == NULL)
    {
        flags = wxTREE_HITTEST_NOWHERE;
...
bool MyGenericTreeCtrl::GetBoundingRect(const wxTreeItemId& item,
                                        wxRect& rect,
                                        bool WXUNUSED(textOnly)) const
{
    wxCHECK_MSG( item.IsOk(), FALSE, _T("invalid item in MyGenericTreeCtrl::GetBoundingRect") );
...
void MyGenericTreeCtrl::Edit( const wxTreeItemId& item )
{
    wxCHECK_RET( item.IsOk(), _T("can't edit an invalid item") );

    MyGenericTreeItem *itemEdit = (MyGenericTreeItem *)item.m_pItem;

    MyTreeEvent te( wxEVT_COMMAND_TREE_BEGIN_LABEL_EDIT, GetId() );
    te.m_item = (void*) itemEdit;
    te.SetEventObject( this );
    ...
    m_textCtrl = new MyTreeTextCtrl(this, itemEdit);
...
wxTextCtrl* MyGenericTreeCtrl::GetEditControl() const
{
    return m_textCtrl;
}

bool MyGenericTreeCtrl::OnRenameAccept(MyGenericTreeItem *item,
                                       const wxString& value)
{
    MyTreeEvent le( wxEVT_COMMAND_TREE_END_LABEL_EDIT, GetId() );
    le.m_item = (void*) item;
...
void MyGenericTreeCtrl::OnRenameCancelled(MyGenericTreeItem *item)
{
...
void MyGenericTreeCtrl::OnRenameTimer()
{
    Edit( m_current );
}

void MyGenericTreeCtrl::OnMouse( wxMouseEvent &event )
{
	//static int ii = 0;
	//printf( "MOUSE %d\n", ii++ );
    if ( !m_anchor ) return;
    ...
    if ( !(event.LeftDown() ||
           event.LeftUp() ||
           //event.RightDown() || mp: own event handling in MView
           event.LeftDClick() ||
           event.Dragging() ||
           ((event.Moving() || event.RightUp()) && m_isDragging)) )
    {
				event.ResumePropagation( wxEVENT_PROPAGATE_MAX ); // mp
        event.Skip();
    ...
    int flags = 0;
    MyGenericTreeItem *item = m_anchor->HitTest(pt, this, flags, 0);
    ...
    if ( event.Dragging() && !m_isDragging )
    {
        ...
        MyTreeEvent nevent( command, GetId() );
        nevent.m_item = (void*) m_current;
        ...
        if ( GetEventHandler()->ProcessEvent(nevent) && nevent.IsAllowed() )
        {
            ...
            {
                m_oldSelection = (MyGenericTreeItem*) GetSelection().m_pItem;
    ...
    else if ( (event.LeftUp() || event.RightUp()) && m_isDragging )
    {
        ...
        if ( m_oldSelection )
        {
            ...
            RefreshLine(m_oldSelection);
            m_oldSelection = (MyGenericTreeItem *)NULL;
        }

        // generate the drag end event
        MyTreeEvent event(wxEVT_COMMAND_TREE_END_DRAG, GetId());
        ...
        m_isDragging = FALSE;
        m_dropTarget = (MyGenericTreeItem *)NULL;
    ...
    else
    {
        ...
        if ( event.RightDown() )
        {
            MyTreeEvent nevent(wxEVT_COMMAND_TREE_ITEM_RIGHT_CLICK, GetId());
            nevent.m_item = (void*) item;
        ...
        else if ( event.LeftUp() )
        {
            if ( m_lastOnSame )
            {
                     ...
                     HasFlag(wxTR_EDIT_LABELS) )
                {
                    ...
                    {
                        m_renameTimer = new MyTreeRenameTimer( this );
                    }

                    m_renameTimer->Start( MyTreeRenameTimer::DELAY, TRUE );
                }
        ...
        else // !RightDown() && !LeftUp() ==> LeftDown() || LeftDClick()
        {
            ...
            if ( event.LeftDClick() )
            {
                ...
                MyTreeEvent nevent( wxEVT_COMMAND_TREE_ITEM_ACTIVATED, GetId() );
                nevent.m_item = (void*) item;
                nevent.m_pointDrag = CalcScrolledPosition(pt);
                nevent.SetEventObject( this );
...
void MyGenericTreeCtrl::OnIdle( wxIdleEvent &WXUNUSED(event) )
{
...
void MyGenericTreeCtrl::CalculateLevel( MyGenericTreeItem *item, wxDC &dc, int level, int &y )
{
    if ( item->IsHidden() ) return;
    int x = level*m_indent;
    if (!HasFlag(wxTR_HIDE_ROOT))
...
void MyGenericTreeCtrl::CalculatePositions()
{
    if ( !m_anchor ) return;
...
void MyGenericTreeCtrl::RefreshSubtree(MyGenericTreeItem *item)
{
    if (m_dirty) return;
...
void MyGenericTreeCtrl::RefreshLine( MyGenericTreeItem *item )
{
    if (m_dirty) return;
...
void MyGenericTreeCtrl::RefreshSelected()
{
...
void MyGenericTreeCtrl::RefreshSelectedUnder(MyGenericTreeItem *item)
{
    if ( item->IsSelected() )
        RefreshLine(item);

    const MyArrayGenericTreeItems& children = item->GetChildren();
    size_t count = children.GetCount();
    for ( size_t n = 0; n < count; n++ )
    {
...
bool MyGenericTreeCtrl::SetBackgroundColour(const wxColour& colour)
{
...
bool MyGenericTreeCtrl::SetForegroundColour(const wxColour& colour)
{
...
IMPLEMENT_DYNAMIC_CLASS(MyTreeEvent, wxNotifyEvent)


MyTreeEvent::MyTreeEvent(wxEventType commandType, int id)
